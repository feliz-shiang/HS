...
@interface GYEPSaleAfterApplyForOnlyReturnGoodsMoneyViewController ()<UITableViewDataSource, UITableViewDelegate, CellForReturnGoodsCellDelegate, UITextViewDelegate>
{
...
//    self.arrDataSource = self.dicDataSource[@"items"];// [NSMutableArray arrayWithObjects:@"", @"", nil];
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(vRow1Bkg.frame.origin.x,
                                                                  vRow1Bkg.frame.origin.y,
                                                                  vRow1Bkg.frame.size.width,
...
//    [ivUploadPicture moveX:0 moveY:moveY];
    ...
    self.lab.frame=CGRectMake(15, scroll.frame.origin.y+scroll.bounds.size.height+5, self.view.frame.size.width-30, 50);
...
//    [self   addGestureToImgView ];
    ...
    {
        [Utils showMessgeWithTitle:@"提示" message:kLocalized(@"该订单没有可以进行退货退款的商品.") isPopVC:self.navigationController];
    } 
...
//    tapForPic.tag=100;
//    [ivUploadPicture addGestureRecognizer:tapForPic];
//}
...
{
    NSString *str = [NSString stringWithFormat:@"<font size=13 color=red>%@</font>", amount];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    ivup.image = savedImage;    //            activityIndicatorView.hidden = NO;
...
//
//    [activityIndicatorView stopAnimating];
//    DDLogInfo(@"上传的图片url:%@", uploadPicReturnURL);
////    activityIndicatorView.hidden = YES;
...
- (void)updateAmountAndItemIdsBySelect
{
    ...
    for (int i = 0; i < self.arrDataSource.count; i++)
    {
        ...
        if (cell.selected)
        {
            totalAmount += kSaftToCGFloat(self.arrDataSource[i][@"subTotal"]);
            totalAmountPv += kSaftToCGFloat(self.arrDataSource[i][@"subPoints"]);
    ...
    [self setReturnAmount:[Utils formatCurrencyStyle:totalAmount]];
}
...
- (void)applyFor
{
    ...
    NSDictionary *allParas = @{@"key": data.ecKey,
                               ...
                               @"price":strReturnAmount,
                               @"points":strReturnPvAmount,
                               @"picUrls":urlString
                               };
