...
@interface GYPointToHSDViewController ()<UITextFieldDelegate>
{
...
- (void)viewDidLoad
{
    ...
    viewRowInputPointAmount.lbLeftlabel.text = kLocalized(@"number_of_point_to_hsd");
...
- (void)btnNextClick:(id)sender
{
        ...
        inputValue > 0)//输入合法
    {
        ...
        {
            NSString *message = [NSString stringWithFormat:@"积分转互生币积分数为不小于%@的整数", [Utils formatCurrencyStyle:data.user.minPointToCashAmount]];
...
//            [alert show];
        ...
        {
            NSString *message = @"积分转互生币积分数必须为整数.";
            [Utils alertViewOKbuttonWithTitle:@"提示" message:message];
            return;
        }
        ...
        nextVC.navigationItem.title = kLocalized(@"point_to_hsd_confirm");
        
        nextVC.inputValue = inputValue;
//        nextVC.strPointToHSD = viewRowInputPointAmount.tfRightTextField.text;
...
//    double cash = inputValue * data.user.pointToHSDRate;
...
- (void)setValues
{
    ...
    lbTipMin1.text = kLocalized(@"point_to_hsd_tip1");
    lbTipMin2.text = [NSString stringWithFormat:kLocalized(@"point_to_hsd_tip2")
                      , [Utils formatCurrencyStyle:data.user.minPointToHSD]];

//    lbTipMin.text = [NSString stringWithFormat:kLocalized(@"point_to_hsd_tip")
