...
- (void)viewDidLoad {
    ...
    if (_showIndexOfCount)
    {
        lbShowIndexOfCount = [[UILabel alloc] initWithFrame:CGRectMake(pagingScrollViewFrame.origin.x,
                                                                       CGRectGetMaxY(pagingScrollViewFrame) - 50,
                                                                       pagingScrollViewFrame.size.width,
                                                                       30)];
        [lbShowIndexOfCount setTextColor:[UIColor whiteColor]];
        [lbShowIndexOfCount setTextAlignment:UITextAlignmentCenter];
        [lbShowIndexOfCount setBackgroundColor:[UIColor clearColor]];
        [lbShowIndexOfCount setText:@""];
        [self.view addSubview:lbShowIndexOfCount];
    }
    ...
    }else if (self.customRigthButton)//自定义
    {
//        _actionButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAction target:self action:@selector(customActionButtonPressed:)];
        UILongPressGestureRecognizer * pgr = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(longCustomActionButtonPressed:)];
        pgr.delegate = self;
...
- (void)updateNavigation {
    ...
    lbShowIndexOfCount.text = [NSString stringWithFormat:@"%lu/%lu", (unsigned long)(_currentPageIndex+1), (unsigned long)numberOfPhotos];
...
-(void)longCustomActionButtonPressed:(UILongPressGestureRecognizer *)sender
{
    if (sender.state == UIGestureRecognizerStateBegan)
    {
        [self customActionButtonPressed:nil];
    }
}

- (void)customActionButtonPressed:(id)sender
{
    [UIActionSheet presentOnView:self.view withTitle:nil otherButtons:@[@"保存到相册"] onCancel:^(UIActionSheet *sheet) {
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    ...
    [self showProgressHUDCompleteMessage: error ? NSLocalizedString(@"图片保存失败", @"Informing the user a process has failed") : NSLocalizedString(@"图片已存入手机相册.", @"Informing the user an item has been saved")];
